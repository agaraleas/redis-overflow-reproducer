...
static uint64_t calcNextExecutionPeriod(uint64_t expiretime) {
    /* Next execution period should not be negative since it is returned as uint64_t and should also be greater than 0 after conversion to ms.
       To avoid casting to long long (for handling negative result of substraction) and a second if clause (for hanling the ms conversion), 
       we can compare if the expire time would be at least 1000 microseconds greater than now in a single statement */
    uint64_t now = ustime();
...
static aeTimeEventHandling moduleTimerHandler(struct aeEventLoop *eventLoop, long long id, void *clientData) {
    ...
    raxStop(&ri);
    printf("Calculated next period: %lld\n", handling.nextExecutionPeriod);
